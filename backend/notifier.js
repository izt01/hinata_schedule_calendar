...
function mailWrap(color, icon, title, body, extra) {
  ...
  return '<div style="font-family:sans-serif;max-width:480px;margin:0 auto">'
    ...
    + '<div style="background:#f8f8ff;padding:24px;border-radius:0 0 16px 16px;border:1.5px solid #e0e8ff;border-top:none">'
      + body + extra
      + '<p style="font-size:11px;color:#aaa;margin-top:24px;border-top:1px solid #eee;padding-top:12px">日向坂46 ひなたカレンダー</p>'
    + '</div>'
    + '</div>';
}

function stepCard(title, dueDate, borderColor, labelColor) {
  borderColor = borderColor || '#c8e0f4';
  labelColor  = labelColor  || '#e05050';
  var due = dueDate
...
async function notifyAssignment({ pool, userId, scheduleTitle, stepTitle, dueDate }) {
  ...
  const html = mailWrap(
    ...
    '<p style="color:#333;font-size:14px;line-height:1.8"><b>' + nickname + '</b> さん、<br>スケジュール「<b>' + scheduleTitle + '</b>」で新しいタスクが割り当てられました。</p>',
...
async function sendDueReminders(pool) {
  ...
  const nowHour   = nowJST.getUTCHours();
  ...
  console.log('[Reminder] 実行: ' + todayStr + ' ' + nowHour + '時');
  ...
  if (!targets.length) { console.log('[Reminder] 送信時刻でないためスキップ'); return; }
  ...
  for (const { dueStr, isToday } of targets) {
    const res = await pool.query(`
      ...
      WHERE ss.status = 'in_progress' AND ss.due_date = $1 AND ss.assignee_id IS NOT NULL
    `, [dueStr]);
    ...
    for (const row of res.rows) {
      ...
      const html = mailWrap(
        color, '⏰', 'タスクの期限' + (isToday ? 'は今日' : 'は明日') + 'です',
        '<p style="color:#333;font-size:14px;line-height:1.8"><b>' + row.nickname + '</b> さん、<br>スケジュール「<b>' + row.schedule_title + '</b>」のタスクの期限が近づいています。</p>',
